...
export function Timeline({leftList, rightList}) {
    ...
    return (
            <div className={timelineStyling}>
                ...
                {
                    leftList?.map((nextDataElement) => <h5 className="first:mb-4 sm:first:mb-0" key={nextDataElement}>{nextDataElement}</h5>)
                }
                </div>
                <div className="vertical-line"></div>
                <div className="flex-[3] flex flex-col justify-between gap-6 py-4 sm:flex-[2]">
                {
                    rightList?.map((nextDataElement) => <h5 className="last:text-action" key={nextDataElement}>{nextDataElement}</h5>)
                }
                </div>
            </div>
    );
